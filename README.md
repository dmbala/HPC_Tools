...
| Dir | Run where | Needs |
|---|---|---|
| `slurm/` | any node with the Slurm client | `sacct` (and `scontrol` for `stotal_kempner`); Python 3 standard library only. **`seff_history`** additionally needs `numpy`, `pandas`, and `termplotlib` — its shebang points at the FASRC `seff-array` env (`/n/sw/envs/seff-array/bin/python3`); repoint it if that env isn't on your cluster. **`gpu_idle_now`** additionally needs the jobstats `config` module + `requests` (it queries Prometheus like the `gpu/` window tools) and `gpu/gpulib.py` alongside in the repo. |
